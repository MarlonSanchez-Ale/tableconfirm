  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        `/api/getTable?nombres=${encodeURIComponent(nombres)}&apellidos=${encodeURIComponent(apellidos)}`,
        {
          method: "GET",
      ...
      <div className="bg-white rounded-2xl shadow-lg w-full text-center relative overflow-hidden flex flex-col items-center">
        ...
        <div className="p-10 flex flex-col justify-center md:mt-28 sm:mt-20 gap-3 text-center">
          <div className="flex flex-col">
            ...
            <p className="text-gray-600 text-lg">Ingresá tu nombre y apellido para obtener tu número de mesa</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-3 w-full">
            <input
              type="text"
              placeholder="Nombres"
              className="w-full px-4 py-2 border rounded-lg shadow-md text-brown-700"
              value={nombres}
